...
class SimpleMultiTimeframeDataManager:
    """簡化版多時間框架資料管理器"""
...
class SimpleLiquidityGrabDetector:
    """簡化版流動性獵取檢測器"""
...
class SimpleMultiTimeframeSignalGenerator:
    """簡化版多時間框架訊號生成器"""
    
    def __init__(self):
        self.order_block_detector = SimpleOrderBlockDetector()
        self.fvg_detector = SimpleFVGDetector()
        self.liquidity_grab_detector = SimpleLiquidityGrabDetector()
...
class SimpleShortTermTradingSystem:
    """簡化版短線交易系統"""
    ...
    def __init__(self, symbol: str = 'AAPL', start_date: str = '2024-01-01', 
        ...
        # 初始化組件
        self.data_manager = SimpleMultiTimeframeDataManager(symbol, start_date, end_date)
        self.signal_generator = SimpleMultiTimeframeSignalGenerator()
    ...
    def plot_results(self):
        ...
        ax1.set_title(f'{self.symbol} 簡化版多時間框架SMC短線交易結果', fontsize=14, fontweight='bold')
        ax1.set_ylabel('價格')
        ax1.legend()
        ...
        plt.tight_layout()
        plt.savefig(f'{self.symbol}_simple_multi_timeframe_trading_results.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def save_results(self, performance: Optional[Dict]):
        ...
        if self.trades:
            df_trades = pd.DataFrame(self.trades)
            df_trades.to_excel(f'{self.symbol}_simple_multi_timeframe_trades.xlsx', index=False)
            print(f"✅ 交易記錄已儲存至 {self.symbol}_simple_multi_timeframe_trades.xlsx")
        
        # 儲存訊號記錄
        if self.signals:
            df_signals = pd.DataFrame(self.signals)
            df_signals.to_excel(f'{self.symbol}_simple_multi_timeframe_signals.xlsx', index=False)
            print(f"✅ 訊號記錄已儲存至 {self.symbol}_simple_multi_timeframe_signals.xlsx")
        ...
        if performance:
            ...
            performance_df.to_excel(f'{self.symbol}_simple_multi_timeframe_performance.xlsx', index=False)
            print(f"✅ 績效摘要已儲存至 {self.symbol}_simple_multi_timeframe_performance.xlsx")

# 主程式執行
if __name__ == "__main__":
    # 建立短線交易系統實例
    trading_system = SimpleShortTermTradingSystem(
