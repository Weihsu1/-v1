...
class MultiTimeframeSignalGenerator:
    ...
    def generate_signals(self, data: Dict[str, pd.DataFrame]) -> List[Dict]:
        ...
        # 過濾和優化訊號
        filtered_signals = self._filter_signals(all_signals, data)
        ...
        for lg in liquidity_grabs:
            if lg['idx'] < len(df) - 1:
                signals.append({
                    ...
                    'macd': df.iloc[lg['idx']]['MACD'],
                    'atr': df.iloc[lg['idx']]['ATR']
                })
        
        return signals
    
    def _filter_signals(self, signals: List[Dict], data: Dict[str, pd.DataFrame]) -> List[Dict]:
        """過濾和優化訊號"""
        filtered_signals = []
        
        for signal in signals:
            # 基本過濾條件
            if (signal['rsi'] is not None and 
                signal['macd'] is not None and 
                signal['atr'] is not None):
